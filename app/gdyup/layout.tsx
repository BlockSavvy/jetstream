...
// Also protect /gdyup/payment/* routes
...
export default function GdyupLayout({ children }: { children: ReactNode }) {
  ...
  const isPaymentPath = pathname ? pathname.startsWith('/gdyup/payment/') : false;
  const isDashboardPath = pathname ? pathname.startsWith('/gdyup/dashboard') : false;
  const isOfferPath = pathname ? (
    pathname.startsWith('/gdyup/offer/') && 
  ...
  useEffect(() => {
    console.log('GDYUP Layout Auth State:', { 
      pathname, 
  ...
  useEffect(() => {
    const checkAndRefreshAuth = async () => {
      ...
      console.log('GDYUP: Running enhanced auth check');
      ...
      try {
        ...
        if (storedUserId) {
          ...
          console.log('GDYUP: Found user_id in localStorage:', localUserId);
        } 
        
        if (tokenData) {
          localTokenData = tokenData;
          console.log('GDYUP: Found token data in localStorage');
          ...
          if (!localUserId) {
            try {
              ...
              if (parsed && parsed.user && parsed.user.id) {
                ...
                console.log('GDYUP: Extracted user_id from token data:', localUserId);
                ...
                try {
                  ...
                  if (!error && data.session?.user?.id) {
                    ...
                    localStorage.setItem('jetstream_user_id', localUserId);
                    console.log('GDYUP: Retrieved user_id from token:', localUserId);
                  }
                } catch (tokenError) {
                  console.warn('GDYUP: Error decoding token:', tokenError);
                }
              }
            } catch (parseErr) {
              console.warn('GDYUP: Error parsing token data:', parseErr);
            }
          }
        }
      } catch (storageErr) {
        console.warn('GDYUP: Error accessing localStorage:', storageErr);
      }

      // If we have a local user ID but no user object, or we're on a protected route
      if ((localUserId || localTokenData) && (!user || needsAuth)) {
        console.log('GDYUP: Have local auth data but no user object, or on protected route. Refreshing session...');
        ...
        try {
          ...
          if (refreshResult) {
            console.log('GDYUP: Session refreshed successfully via AuthProvider');
            ...
            console.log('GDYUP: Standard refresh failed, trying with stored token...');
            ...
            try {
              ...
              if (parsed && parsed.access_token && parsed.refresh_token) {
                ...
                if (!error && data.session) {
                  console.log('GDYUP: Session restored successfully via stored token');
                  toast.success('Session restored', { id: 'auth-refresh' });
                  
                  // Wait for auth state to update
                  await new Promise(resolve => setTimeout(resolve, 800));
                } else {
                  console.warn('GDYUP: Error restoring session with stored token:', error);
                }
              }
            } catch (tokenError) {
              console.warn('GDYUP: Error using stored token:', tokenError);
            }
          }
          ...
          if (isNonPaymentProtectedRoute && !user && !authLoading && !redirectAttempted.current) {
            console.log('GDYUP: Auth failed for protected route, considering redirect...');
            ...
            setTimeout(async () => {
              ...
              if (!data.session) {
                console.log('GDYUP: Still no auth after delay, redirecting to login');
                redirectAttempted.current = true;
                
                // Store current path for redirect back after login
                try {
                  sessionStorage.setItem('auth_redirect_url', pathname || '/gdyup');
                } catch (e) {
                  console.warn('GDYUP: Could not set redirect URL:', e);
                }
                
                toast.error('Please sign in to continue', { id: 'auth-refresh' });
                router.push(`/auth/login?returnUrl=${encodeURIComponent(pathname || '/gdyup')}&t=${Date.now()}`);
              } else {
                console.log('GDYUP: Auth restored after delay, no redirect needed');
                toast.success('Session restored', { id: 'auth-refresh' });
              }
            }, 1500); // 1.5 second delay
          }
        } catch (refreshError) {
          console.error('GDYUP: Error during auth refresh:', refreshError);
  ...
  useEffect(() => {
    ...
    if (isPaymentPath) {
      console.log('GDYUP: Payment path detected, bypassing auth redirect check');
      return;
    }
    
    // Only redirect if authentication is required for this route
    if (!authLoading && needsAuth && !user && !redirectAttempted.current) {
      console.log(`GDYUP: Protected route ${pathname} requires login`);
