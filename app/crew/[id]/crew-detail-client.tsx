...
export default function CrewDetailClient({ crew }: CrewDetailClientProps) {
  ...
  const getProfileImageUrl = () => {
    ...
    if (updatedCrew.profileImageUrl.startsWith('http')) {
      ...
      // For relative paths from the public directory
      // Try to handle different extensions if path doesn't include extension
      const imagePath = updatedCrew.profileImageUrl;
      
      // If the path already has an extension, return it as is
      if (imagePath.endsWith('.jpg') || imagePath.endsWith('.jpeg') || 
          imagePath.endsWith('.png') || imagePath.endsWith('.webp')) {
        return imagePath;
      }
      
      // If no extension, try to be smart about it
      // For captain_powell and captain_reid, we know they have .jpg extensions
      if (imagePath.includes('captain_powell') || imagePath.includes('captain_reid')) {
        return `${imagePath}.jpg`;
      }
      
      // Default to adding .jpg for other cases
      return `${imagePath}.jpg`;
    }
  };
